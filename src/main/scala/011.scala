...
object _011 {
  ...
  def parseString(toProcess: String): Array[Array[Long]] = {
    ...
    val pattern = newline + "|" + delimiter
    // debug
    println("Pattern is \"" + pattern + "\"")
    // enddebug
    val rows = toProcess.split(pattern)
    // debug
    println("Got " + rows.length + " rows")
    println("First row is: " + rows(0))
    // enddebug
    val cells = rows.map(_.split("\\s+").map(java.lang.Long.parseLong(_.trim)))
    // debug
    println("First row has " + rows(0).length + " cells")
    println("First cell of first row is " + rows(0)(0))
    // enddebug
    cells
  }
